...
struct VideoState
{
	...
	int queue_picture(AVFrame *pFrame,VideoState *myState);
...
typedef void(* CKMirroVideoBegin)(int width, int height, const void *buffer, int buflen, int payloadtype, double timestamp);
typedef void(* CKMirroVideoProcess)(int width, int height,const void *buffer, int buflen, int payloadtype, double timestamp);
typedef void(* CKMirroVideoStop)();

typedef void(* CKMirroAudioBegin)(int bits, int channels, int samplerate, int isaudio);
typedef void(* CKMirroAudioProcess)(const void *buffer, int buflen, double timestamp, uint32_t seqnum);
typedef void(* CKMirroAudioStop)();
...
public:
	...
	void mirrorvideoplaybegin(CKMirroVideoBegin 
		block); //开始播放视频
	void mirrorvideoplaying(CKMirroVideoProcess vprocessblock); //播放视频中
	void mirrorvideoplaystop(CKMirroVideoStop vstopblock); //停止播放音频

	void mirroraudioplaybegin(CKMirroAudioBegin abeginblock);  //开始播放音频
	void mirroraudioplaying(CKMirroAudioProcess aprocessblock); //播放音频中
	void mirroraudioplaystop(CKMirroAudioStop astopblock); //停止播放音频
...
public:
	...
	CKMirroVideoBegin   vBegin;
	CKMirroVideoProcess vProcess;
	CKMirroVideoStop    vStop;

	CKMirroAudioBegin   aBegin;
	CKMirroAudioProcess aProcess;
	CKMirroAudioStop    aStop;
