...
#pragma
	...
	const char *hostname = "ckdz_123";
	myhostname = new char[100];
...
//
//int VideoSource::yuv420p2rgb24(AVFrame *pFrame)
//{
//	if (pFrame->data[0]) {
//		int width = pFrame->width;
//		int height = pFrame->height;
//
//		int dstWidth = pFrame->width;
//		int dstHeight = pFrame->height;
//
//		struct SwsContext *scxt = sws_getContext(width, height, (AVPixelFormat)pFrame->format, dstWidth, dstHeight, AV_PIX_FMT_BGR24, SWS_POINT, NULL, NULL, NULL);
//		if (scxt == NULL) {
//			return -1;
//		}
//
//		int det_bpp = av_get_bits_per_pixel(av_pix_fmt_desc_get(AV_PIX_FMT_BGR24));
//		av_image_alloc(pictureData, pictureLineSize, width, height, AV_PIX_FMT_BGR24, 1);
//		sws_scale(scxt, (const uint8_t **)pFrame->data, pFrame->linesize, 0, height, pictureData, pictureLineSize);//3840*720
...
void VideoState::video_thread_loop(VideoState *self)
{
	...
	while (!self->video_quit) {
		...
		//printf("ckdz_debug_video_thread_loop\n");
		...
		if (frameFinished) {
			int ret = 0;
			//printf("ckdz_debug_video_decode_ok!\n");

			ret = self->queue_picture(pFrame, self);
			if (ret < 0)
				break;
		}
		t_end = GetTickCount();
		printf("decode_cust_time = %d\n", t_end- t_start);
...
int VideoState::queue_picture(AVFrame *pFrame, VideoState *myState)
{
	//printf("ckdz_debug_queue_picture_out\n");

	if (this->sws_context == NULL || width != this->codecCtx->width || height != this->codecCtx->height) {
		//printf("ckdz_debug_queue_picture_in_not_nil\n");
...
/*YUV数据翻转*/
		...
		/*YUV转RGB*/
		int det_bpp = 0;
		//printf("ckdz_debug_pFrame->data[0] out\n");
		if (pFrame->data[0]) {
			//printf("ckdz_debug_pFrame->data[0] in\n");
			...
			struct SwsContext *scxt = sws_getContext(width, height, (AVPixelFormat)pFrame->format, dstWidth, dstHeight, AV_PIX_FMT_BGR24, SWS_POINT, NULL, NULL, NULL);
			/*	t_end = GetTickCount();
				printf("ckdz_debug_sws_getContext in%ld\n", t_end - t_start);*/

			if (scxt == NULL) {
				//printf("ckdz_debug_sws_getContext null\n");
				return -1;
			}
			//t_start = GetTickCount();
			det_bpp = av_get_bits_per_pixel(av_pix_fmt_desc_get(AV_PIX_FMT_BGR24));
			/*t_end = GetTickCount();
			printf("ckdz_debug_av_get_bits_per_pixel in%ld\n", t_end - t_start);*/

			//t_start = GetTickCount();
			av_image_alloc(pictureData, pictureLineSize, width, height, AV_PIX_FMT_BGR24, 1);
			/*	t_end = GetTickCount();
				printf("ckdz_debug_av_image_alloc in%ld\n", t_end - t_start);*/
		...
		if (det_bpp)
		{
			//printf("ckdz_debug_det_bpp in\n");

			//width = this->codecCtx->width; //恒屏时宽高变化
			...
			//}
/*
			int length = pictureLineSize[0] * codecCtx->height;*/
			int length = codecCtx->width*codecCtx->height;


			VideoSource *_myThis = (VideoSource *)(myState)->vsource;
			printf("queue_picture_OK!:%p", _myThis);
			_myThis->vProcess(width, height, pictureData[0], width * height * 3, 1, 0.f);
		}
		av_freep(pictureData);
	}
...
void VideoSource::AirPlayOutputFunctions::audio_init(void *cls, int bits, int channels, int samplerate, int isaudio)
{
	...
	VideoSource *myVS = (VideoSource *)cls;
	myVS->aBegin(bits, channels, samplerate, isaudio);
	//myVS->aProcess(buffer, buflen, timestamp, seqnum);
...
void VideoSource::AirPlayOutputFunctions::mirroring_play(void *cls, int width, int height, const void *buffer, int buflen, int payloadtype, double timestamp)
{
	...
	VideoSource *myVS = (VideoSource *)cls;
	myVS->vBegin(width, height, NULL, 0, payloadtype, timestamp);
...
void VideoSource::AirPlayOutputFunctions::mirroring_process(void *cls, const void *buffer, int buflen, int payloadtype, double timestamp)
{
	...
	if (buflen > 0) {
		if (payloadtype == 1)
		{
			...
			//if (frm_node) {
			//	unsigned char *node_buffer = pkt->data;
			//	int node_size = pkt->size;
			//	VideoSource *myVS = (VideoSource *)cls;
			//	myVS->vProcess(node_buffer, node_size, payloadtype, timestamp);
			//}
...
int VideoSource::start_airplay()
{
	...
	return XinDawn_StartMediaServer(myhostname, 768, 1024, &ao);
}
...
void VideoSource::mirrorvideoplaybegin(CKMirroVideoBegin vbeginblock)
{

	vBegin = vbeginblock;
}
void VideoSource::mirrorvideoplaying(CKMirroVideoProcess vprocessblock)
{
	vProcess = vprocessblock;
}
void VideoSource::mirrorvideoplaystop(CKMirroVideoStop vstopblock)
{
	vStop = vstopblock;
}

void VideoSource::mirroraudioplaybegin(CKMirroAudioBegin abeginblock)
{
	aBegin = abeginblock;
}
void VideoSource::mirroraudioplaying(CKMirroAudioProcess aprocessblock)
{
	aProcess = aprocessblock;
}
void VideoSource::mirroraudioplaystop(CKMirroAudioStop astopblock)
{
	aStop = astopblock;
}
