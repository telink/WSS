...
raop_rtp_process_events(raop_rtp_t *raop_rtp)
{
	...
	if (volume_changed) {
		if (raop_rtp->callbacks.audio_set_volume) {
			raop_rtp->callbacks.audio_set_volume(raop_rtp->callbacks.cls, ceil(volume));
		}
	}
...
raop_rtp_thread_udp(void *arg)
{
			...
			if (packetlen >= 12) {
				...
				while ((audiobuf = raop_buffer_dequeue(raop_rtp->buffer, &audiobuflen, no_resend))) {
                    if (raop_rtp->callbacks.audio_process) {
                        raop_rtp->callbacks.audio_process(raop_rtp->callbacks.cls, audiobuf, audiobuflen, 0.0, 1);
                    }
				}
	...
	if (raop_rtp->callbacks.audio_destroy) {
        raop_rtp->callbacks.audio_destroy(raop_rtp->callbacks.cls);
    }
	return 0;
}
...
raop_rtp_set_volume(raop_rtp_t *raop_rtp, float volume)
{
	...
	volume = (144.0 - (volume * -1))*128/144;

	logger_log(raop_rtp->logger, LOGGER_INFO, "set volume: %f", volume);
