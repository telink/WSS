...
DWORD __stdcall airdata_thread_proc(LPVOID lpParameter) {
	...
	while (1) {
		logger_log(airplay->m_logger, 6, "%s:::%d",__FUNCTION__,__LINE__);
		...
		if (v2) {
			...
			logger_log(airplay->m_logger, 6, "raop.c ret::::%d  socket:::%d  nfds::::%d \n", ret, sock, nfds);
			if (ret) {
				logger_log(airplay->m_logger, 6, "%s:::%d", __FUNCTION__, __LINE__);
				if (ret == -1) {
		...
		if (plist_dict_get_size(p_dict)) {
            ...
            if (p_ekey != NULL) {
				...
				if (p_ekey_data && p_ekey_size == 72) {// ekey 需要通过 DRM 在线解密
					//int key_size = 0;
					int key_size = 16;
					logger_log(conn->raop->logger, LOGGER_DEBUG, "message31111: %s \n", bin2hex(conn->raop->message3, 164));
					unsigned char* key_data = fairplay_decrypt(conn->raop->message3, p_ekey_data);
					logger_log(conn->raop->logger, LOGGER_DEBUG, "message3: %s \n", bin2hex(conn->raop->message3, 164));
...
http_response_t * request_handle_fpsetup(raop_conn_t *conn, http_request_t *request, http_response_t *response, char **pResponseData, int *pResponseDataLen) {
	...
	if (datalen == 16) {
		...
		logger_log(conn->raop->logger, LOGGER_DEBUG, "1buf: %s \n", bin2hex(buf, size));
	}
	else {
		size = 32;
		buf = fairplay_setup(data, datalen);
		logger_log(conn->raop->logger, LOGGER_DEBUG, "2buf: %s \n", bin2hex(buf, size));
		//message3 = data;
		//strncpy(conn->raop->message3,data, datalen);
		memcpy(conn->raop->message3, data, datalen);
		logger_log(conn->raop->logger, LOGGER_DEBUG, "message3111: %s \n", bin2hex(data, datalen));
		logger_log(conn->raop->logger, LOGGER_DEBUG, "message3111: %s \n", bin2hex(conn->raop->message3, datalen));
	}
